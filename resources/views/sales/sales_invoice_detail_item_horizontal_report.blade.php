...
<html>
<head>
    <title>Invoice Detail Menyamping</title>
    <style>
        body {
...
<table>
    <thead>
    <tr>
        <td class="text-center font-bold" colspan="12">Laporan Invoice Penjualan Detail By Customer</td>
    </tr>
    <tr>
            ...
            if (!empty($post->order)) {
                ...
                foreach ($post->invoicedelivery ?? [] as $invoiceDelivery) {
                    $deliveryProducts = !empty($invoiceDelivery->delivery)
                        ...
                        : collect();
                    $items = $items->merge($deliveryProducts);
                }
            }
        @endphp
