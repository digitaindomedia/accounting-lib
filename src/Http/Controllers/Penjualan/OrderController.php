...
class OrderController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        $importLogId = $this->createImportLog(
            $userId,
            TransactionsCode::ORDER_PENJUALAN,
    ...
    public function getImportLogs(Request $request)
    {
        ...
        $query = ImportLog::where('transaction_type', TransactionsCode::ORDER_PENJUALAN)
            ->withCount('details')
    ...
    {
        $id = $request->id;
        $data = ImportLog::where('transaction_type', TransactionsCode::ORDER_PENJUALAN)
            ->with([
                'details',
                'details.salesOrder',
                'details.salesOrder.vendor'
            ])
            ->find($id);
    ...
    public function deleteImportLogData(Request $request)
    {
        ...
        $importLog = ImportLog::where('transaction_type', TransactionsCode::ORDER_PENJUALAN)
            ->with('details')
            ->find($id);
        ...
        foreach ($importLog->details as $detail) {
            ...
            if (!SalesOrder::whereKey($transactionId)->exists()) {
                $missingData++;
                $deletedDetailIds[] = $detail->id;
                continue;
            }

            if ($this->salesDpRepo->countByOrderId($transactionId) > 0) {
                $failedDelete++;
                continue;
            }

            if ($this->salesOrderRepo->destroy($transactionId, $userId)) {
                $successDelete++;
                $deletedDetailIds[] = $detail->id;
            } else {
                $failedDelete++;
