...
class PaymentController extends Controller
{
    ...
    public function destroy(Request $request)
    {
        $id = $request->id;
        try {
            $deleted = $this->paymentRepo->destroy($id, (int) $request->user_id);
            if ($deleted) {
                $this->data['status'] = true;
        ...
        if(count($reqData) > 0){
            foreach ($reqData as $id){
                ...
                if (!$paymentId) {
                    ...
                    Log::error('[PaymentController][deleteAll] ID pembayaran penjualan tidak valid', [
                        'payload_id' => $id,
                        'user_id' => $userId,
                    ]);
                    continue;
                ...
                try {
                    ...
                    if ($deleted) {
                        ...
                        $failedDelete = $failedDelete + 1;
                        Log::error('[PaymentController][deleteAll] Pembayaran penjualan gagal dihapus', [
                    ...
                    $failedDelete = $failedDelete + 1;
                    Log::error('[PaymentController][deleteAll] Error hapus pembayaran penjualan', [
                        'payment_id' => (int) $paymentId,
                        'user_id' => $userId,
