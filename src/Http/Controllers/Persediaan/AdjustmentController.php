...
class AdjustmentController extends Controller
{
    ...
    public function getAllData(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        extract($params);
        $data = $this->adjusmentRepo->getAllDataBy($search,$page,$perpage,$where);
        $total = $this->adjusmentRepo->getAllTotalDataBy($search,$where);
    ...
    public function store(CreateAdjustmentRequest $request): \Illuminate\Http\JsonResponse
    {
        $res = $this->adjusmentRepo->store($request);
        if($res){
            $this->data['status'] = true;
    ...
    {
        $res = $this->adjusmentRepo->findOne($request->id,array(),['warehouse','coa_adjustment','adjustmentproduct','adjustmentproduct.product','adjustmentproduct.coa','adjustmentproduct.unit']);
        if($res){
            $this->data['status'] = true;
            $this->data['message'] = 'Data berhasil ditemukan';
            $this->data['data'] = $res;
    ...
    public function destroy(Request $request)
    {
        ...
        try
        {
            $this->adjusmentRepo->deleteAdditional($id);
            $this->adjusmentRepo->delete($id);
            DB::commit();
            $this->data['status'] = true;
    ...
    private function prepareExportData(Request $request)
    {
        ...
        extract($params);
        $total = $this->adjusmentRepo->getAllTotalDataBy($search,$where);
        $data = $this->adjusmentRepo->getAllDataBy($search,$page,$total,$where);
        return $data;
    }
    ...
    private function downloadPdf(Request $request, $data, $params, $filename){
        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('accounting::stock.adjustment_stock_report', [
            'data' => $data,
            'params' => $params,
