...
class MutationController extends Controller
{
    ...
    public function show(Request $request): JsonResponse
    {
        ...
        $res = $this->mutationRepo->findOne($id, [], ['fromwarehouse', 'mutation', 'mutation.mutationproduct', 'mutation.mutationproduct.product', 'mutation.mutationproduct.unit','towarehouse', 'mutationproduct','salesquotation', 'mutationproduct.product', 'mutationproduct.unit']);
        if ($res) {
            $this->data['status'] = true;
            $this->data['message'] = 'Data berhasil ditemukan';
            $this->data['data'] = $res;
    ...
    public function destroy(Request $request): JsonResponse
    {
        ...
        try {
            $deleted = $this->mutationRepo->destroy((int) $id, (int) $request->user_id);
            if ($deleted) {
                $this->data['status'] = true;
    ...
    public function deleteAll(Request $request): JsonResponse
    {
        ...
        if (count($reqData) > 0) {
            foreach ($reqData as $id) {
                try {
                    $mutationId = is_array($id) ? ($id['id'] ?? null) : ($id->id ?? $id);
                    if (!$mutationId) {
                        $failedDelete++;
                        continue;
                    }

                    $deleted = $this->mutationRepo->destroy((int) $mutationId, (int) $request->user_id);
                    if ($deleted) {
                        $successDelete++;
                    } else {
                        $failedDelete++;
