...
class PurchasePaymentImport implements ToCollection
{
    protected $userId;
    ...
    {
        $this->userId = $userId;
        $this->paymentRepo = new PaymentRepo(new PurchasePayment(), app(ActivityLogService::class));
    }
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            $supplierCode = trim((string)($row[2] ?? ''));
            ...
            if (!isset($groupedPayments[$paymentNo])) {
                ...
                if ($group['vendor_id'] !== $vendor->id) {
                    $this->errors[] = "Baris {$rowNo}: No pembayaran {$paymentNo} memiliki kode supplier berbeda.";
                    continue;
                }
    ...
    private function validateRow(
        ...
        if (PurchasePayment::where('payment_no', $paymentNo)->exists()) {
        ...
        if ($supplierCode === '') {
            $this->errors[] = "Baris {$rowNo}: Kode supplier kosong.";
            return null;
        }

        $vendor = Vendor::where('vendor_code', $supplierCode)
            ->where('vendor_type', VendorType::SUPPLIER)
            ->first();
        if (empty($vendor)) {
            $this->errors[] = "Baris {$rowNo}: Kode supplier {$supplierCode} tidak ditemukan.";
            return null;
        }
        ...
        $invoice = PurchaseInvoicing::where('invoice_no', $invoiceNo)->first();
        if (empty($invoice)) {
        ...
        if ((int)$invoice->vendor_id !== (int)$vendor->id) {
            $this->errors[] = "Baris {$rowNo}: Invoice {$invoiceNo} bukan milik supplier {$supplierCode}.";
            return null;
        }
